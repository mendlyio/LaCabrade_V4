...
/**
 * POST /admin/odoo/resync-prices
 * Re-synchronise uniquement les PRIX des produits déjà importés depuis Odoo (SSE)
 */
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  try {
    ...
    console.log(`💰 [ADMIN] Re-synchronisation des prix des produits Odoo déjà importés`)
    ...
    if (totalCreated > 0 || totalUpdated > 0) {
      ...
      console.log(`🔄 [CACHE] Cache invalidé après re-synchronisation des prix`)
    }
    ...
    console.error("❌ [ADMIN] Erreur re-synchronisation prix:", error)

    if (!res.headersSent) {
      return res.status(500).json({
        success: false,
        message: "Erreur lors de la re-synchronisation des prix",
        error: error.message,
      })
    }
