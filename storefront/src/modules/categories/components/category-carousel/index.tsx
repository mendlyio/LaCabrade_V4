          ...
          {categories.map((category) => (
            ...
            >
              <LocalizedClientLink
                ...
                className="group relative block overflow-hidden rounded-xl aspect-square bg-gradient-to-br from-gray-100 to-gray-200 hover:shadow-2xl transition-all duration-300"
              >
                {/* Image de fond (première image du premier produit de la catégorie si disponible) */}
                {category.products?.[0]?.thumbnail ? (
                  <div
                    className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
                    style={{
                      backgroundImage: `url(${category.products[0].thumbnail})`
                    }}
                  />
                ) : (
